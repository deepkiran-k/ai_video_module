...
DESCRIPTION:
- This is the main executable script for the "direct" pipeline.
- It runs all steps in order: OCR, Analysis, AI (veo3 plan, summary, 
  selection, script), Voiceover, and final Video Editing.
'''
import sys
...
def run_full_pipeline(video_path_str: str, user_prompt: str):
    ...
    # --- NEW: Step 3.5: Generate Intro Plan ---
    print("\n--- Step 3.5: Generating Intro Plan ---")
    # Define the output path for the generated clip
    intro_clip_path = Path("generated_clips") / video_name / "shot_01_intro.mp4"
    intro_prompt = ai_manager.create_intro_plan(user_prompt)
    
    # This placeholder function prints the manual instructions for the user
    ai_manager.generate_intro_video(intro_prompt, intro_clip_path)
    # The pipeline continues, assuming the user will create the file.
    # --- End of New Step ---
    ...
    # --- MODIFIED: Pass the intro_clip_path to the editor ---
    editor.create_guide_video(
        ...
        intro_clip_path=str(intro_clip_path) if intro_clip_path.exists() else None
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python run_pipeline_direct.py \"<path_to_video>\" \"<your_prompt>\"")
        sys.exit(1)
